...
export class DialogAddABComponent implements OnInit {
  ...
  addBandera(row_obj) {
    ...
    this.serviceAdd.createBandera(
      { orden: orden_count, bandera: row_obj.name.toUpperCase() },
      () => {}
    );
  }
  addArboladura(row_obj) {
    let orden_count = this.banderas[this.banderas.length - 1].orden + 1;
    this.serviceAdd.createArboladura(
      { codigo: orden_count, arboladura: row_obj.name.toUpperCase() },
      () => {}
    );
  }
  backToEntradas(): void {
